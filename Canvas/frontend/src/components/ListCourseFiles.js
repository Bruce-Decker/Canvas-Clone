...
class ListCourseFile extends Component {
    ...
    async componentDidMount() {
        ...
        const response = await axios.get('/listFiles/' + this.props.match.params.CourseId)
        this.setState({
            items: response.data,
        ...
        return (
            <div className = "pageDesign">
              ...
              {  this.state.items.map(item =>  
               <div class="card">
               <div class="card-body">
                <Link to = {`/viewEachCourseFile/${this.props.match.params.CourseId}/${item.item_name}`}> <h3> {item.item_name}</h3></Link> 
