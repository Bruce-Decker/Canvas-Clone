...
router.post('/sendMessage', passport.authenticate('jwt', { session: false }), function(req, res) {
    kafka.make_request('message', {"method": "post_sendMessage", "message": req.body}, function(error, result) {
        if (error) {
            console.log(error)
...
router.get('/getMessages/:receiver_email', passport.authenticate('jwt', { session: false }), function(req, res) {
    kafka.make_request('message', {"method": "get_getMessages", "receiver_email": req.params.receiver_email}, function(error, result) {
        if (error) {
            console.log(error)
            res.status(400).json({msg: 'message not found'});
        } else {
...
router.get('/getExchangedMessages/:receiver_email/:sender_email/:subject', passport.authenticate('jwt', { session: false }), function(req, res) {
    kafka.make_request('message', {"method": "getExchangedMessages", "receiver_email": req.params.receiver_email, "sender_email": req.params.sender_email, "subject": req.params.subject}, function(error, result) {
        if (error) {
            console.log(error)
            res.status(400).json({msg: 'message not found'});
        } else {
